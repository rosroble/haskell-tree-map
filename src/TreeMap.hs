module TreeMap
    (Tree, put, get, size, removeKey, containsKey, mapFromList, filterMap, foldlMap, foldrMap, mapMap, mergeMaps) where
...
removeRoot (Node kv left right) = Node (kv') (left) (right')
    where kv' = keyvalue $ leftmost right
...
mapFromList = foldr putPair Nil where
    putPair (k, v) = put k v
...
filterMap predi tree = filterHelper predi tree Nil where
